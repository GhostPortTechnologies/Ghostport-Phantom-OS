...
def _load_ghostport_apps():
    """Source-of-truth import: pull DESKTOP_APPS from gp-desktop-icons.py
    via importlib so the drawer never drifts from what the desktop renders."""
    import importlib.util
    path = "/opt/ghostport/desktop/gp-desktop-icons.py"
    spec = importlib.util.spec_from_file_location("_gpdi", path)
    mod = importlib.util.module_from_spec(spec)
    # The module guards main() under __name__ == "__main__"; loading just
    # populates module-level constants like DESKTOP_APPS.
...
def parse_desktop_file(path):
    ...
    for line in data.splitlines():
        ...
        if key == "Name" and name is None:
            ...
            tokens = [t for t in shlex.split(val, posix=True) if not (t.startswith("%") and len(t) == 2)]
            exec_cmd = tokens
...
def lookup_ghostport_icon(icon_file):
    ...
    if not os.path.isfile(path):
        return None
    try:
        return GdkPixbuf.Pixbuf.new_from_file_at_scale(path, ICON_PIXEL_SIZE, ICON_PIXEL_SIZE, True)
    except Exception:
        return None
...
class AppDrawer(GhostPortApp):
    ...
    def build_ui(self):
        ...
        self.search_entry.connect("search-changed", lambda _e: self.flowbox.invalidate_filter())
