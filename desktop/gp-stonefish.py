...
class StonefishApp(GhostPortApp):
    ...
    HELP_SECTIONS = [
        ...
        ("Block MAC (right-click action)",
         ...
         "a bare MAC address.\n\n"
         "What blocking does: adds the MAC to the `blocked_macs` set in the "
         "`inet phantom_blocklist` nftables table, which has a forward-hook "
         "drop rule. The device can't reach anything through GhostPort — not "
         "even DHCP. The block is kernel-enforced, not DNS-level.\n\n"
